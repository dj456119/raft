...
func init() {
	if raw := os.Getenv("RAFT_NET_RECEIVE_BUFFER_SIZE"); raw != "" {
		size, _ := strconv.ParseInt(raw, 10, 32)

		if size > 4096 {
			connReceiveBufferSize = int(size)
		}
	}
}
...
func (n *NetworkTransport) handleConn(connCtx context.Context, conn net.Conn) {
	...
	if connReceiveBufferSize > 4096 {
		n.logger.Debug("connection receive buffer size increased from default", "size", connReceiveBufferSize)
	}
	w := bufio.NewWriter(conn)
